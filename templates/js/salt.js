...
async function initApp() {
    try {
        ...
        const wasm = await import('./pkg/salt.js');
        await wasm.default();
...
function renderSvg() {
    ...
    container.innerHTML = saltApp.render_svg(Math.floor(width), Math.floor(height));
}
...
function handleEvent(event) {
    ...
    // Pass event to Rust
    const stateChanged = saltApp.handle_mouse_event(
        event.type,
        x,
        y
    );
    ...
    resizeObserver = new ResizeObserver(entries => {
        for (const entry of entries) {
            if (entry.target === container && saltApp) {
                ...
                // No need to re-initialize, just re-render with new dimensions
                renderSvg();
            }
        }
    });
